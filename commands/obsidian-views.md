...
2. Run:
   ```bash
   python3 "$HOME/.opencode/skills/obsidian-project-memory/scripts/project_views.py" --cwd "$PWD"
   ```
3. If `mode=all`, run:
   ```bash
   python3 "$HOME/.opencode/skills/obsidian-project-memory/scripts/project_views.py" --cwd "$PWD" --include-canvases
   ```
4. Verify outputs:
   - `Views/papers.base`
