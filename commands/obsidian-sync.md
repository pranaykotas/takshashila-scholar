...
1. Run project detection:
   ```bash
   python3 "$HOME/.opencode/skills/obsidian-project-memory/scripts/project_kb.py" detect --cwd "$PWD"
   ```
2. If the project is unbound but is a research-project candidate, bootstrap it first.
3. Run the real filesystem sync helper:
   ```bash
   python3 "$HOME/.opencode/skills/obsidian-project-memory/scripts/project_kb.py" sync --cwd "$PWD" --scope "$scope"
   ```
...
- In a bound repo, this is the safest manual repair entrypoint for the minimum auto-maintenance surface: `Daily/`, `00-Hub.md`, and repo-local `.opencode/project-memory/` state.
