...
IGNORE_DIRS = {
    ...
    'dist', 'build', 'checkpoints', 'checkpoint', 'cache', '.cache', '.opencode',
    'temp', 'tmp', '.tmp'
}
MAX_LIST_ITEMS = 40
MAX_SYNC_PATHS = 24
...
def note_language_from_project_memory(repo_root: Path, project_id: str) -> str | None:
    memory_path = repo_root / '.opencode' / 'project-memory' / f'{project_id}.md'
...
def registry_path(repo_root: Path) -> Path:
    return repo_root / '.opencode' / 'project-memory' / 'registry.yaml'
...
def bootstrap_project(
    ...
    memory_path = repo_root / '.opencode' / 'project-memory' / f'{project_slug}.md'
...
def resolve_binding(repo_root: Path, project_id: str | None = None) -> ProjectBinding:
    ...
    if not registry.get('projects'):
        raise SystemExit('No registered projects found in .opencode/project-memory/registry.yaml')
...
def lifecycle(repo_root: Path, mode: str, project_id: str | None = None) -> dict[str, Any]:
    ...
    memory_path = repo_root / '.opencode' / 'project-memory' / f'{binding.project_id}.md'
...
def find_canonical_note(repo_root: Path, kind: str, query: str, project_id: str | None = None) -> dict[str, Any]:
    ...
    for path in [
        repo_root / '.opencode' / 'project-memory' / f'{binding.project_id}.md',
