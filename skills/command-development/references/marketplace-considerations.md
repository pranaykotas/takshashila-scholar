...
if [ ! -f ".codex/command-initialized" ]; then
...
Checking for user config: .codex/plugin-name.local.md

if [ -f ".codex/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .codex/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .codex/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .codex/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .codex/plugin-name.local.md to customize"
fi
...
Or set in .codex/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".codex/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .codex/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .codex/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .codex/operation-completed.flag
```
