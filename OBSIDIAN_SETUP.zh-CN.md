...
当 Claude Scholar 运行在一个包含 `.opencode/project-memory/registry.yaml` 的仓库里时，应默认把这个仓库视为已经绑定到 Obsidian 项目知识库，并在任务过程中自动维护它。
...
- `/obsidian-link` — 修复或增强 canonical notes 之间的 wikilink
- `/obsidian-review` — 从项目笔记生成文献综述
- `/obsidian-notes` — 规范化论文笔记并连接到项目上下文
- `/obsidian-note` — 对单个 canonical note 执行 archive / purge / rename
- `/obsidian-project` — detach / archive / purge / rebuild
- `/obsidian-views` — 显式生成可选的 `.base` 视图（以及可选的额外 canvases）

## 已绑定仓库的最小自动维护

当仓库已经通过 `.opencode/project-memory/registry.yaml` 绑定后，Claude Scholar 默认应采用保守的自动维护策略：

- 当本轮对话改变研究状态时，至少检查 `Daily/YYYY-MM-DD.md`，
- 只有顶层项目状态真正变化时才更新 `00-Hub.md`，
- 只要项目状态变化就更新 `.opencode/project-memory/<project_id>.md`，
...
| 项目反复被重新导入 | 检查 `.opencode/project-memory/registry.yaml` 是否存在且 repo_root 是否正确 |
...
- `WSL vault` 作为 **source of truth**：`<repo-root>/obsidian-vault`
- `Windows 本地目录` 作为镜像副本：例如 `<wsl-mounted-windows-vault-path>`
