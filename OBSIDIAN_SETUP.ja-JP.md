...
Claude Scholarが`.codex/project-memory/registry.yaml`を含むリポジトリ内で実行されている場合、そのリポジトリをObsidianプロジェクトナレッジベースにバインド済みとして扱い、デフォルトで更新を行います。
...
1. `.codex/project-memory/registry.yaml`のプロジェクト設定
...
Codex版ではClaude Codeのようにslash commandが前面には出ません。同じワークフローは自然言語プロンプトと対応するskill / agentを通じて利用します。主な入口は次のとおりです:

- プロジェクトナレッジベースのブートストラップまたはインポート
- 新しいMarkdownファイルまたはディレクトリを分類→昇格/マージ/デイリーステージングで取り込み
- 決定論的なファイルシステム同期を強制実行し、ヘルパーノートを更新
- 正規ノート間のwikilinkを修復または強化
- プロジェクトノートから文献合成を生成
- 論文ノートを正規化し、プロジェクトコンテキストに接続
- 単一の正規ノートをアーカイブ/パージ/リネーム
- デタッチ/アーカイブ/パージ/再構築
- オプションの`.base`ビュー（および追加キャンバス）を必要なときだけ生成

## バインド済みリポジトリの最低限のメンテナンス

リポジトリが`.codex/project-memory/registry.yaml`を通じて既にバインドされている場合、Claude Scholarは自動メンテナンスを控えめに行います:

- ターンが研究状態を変更した場合、常に`Daily/YYYY-MM-DD.md`を確認
- `00-Hub.md`はトップレベルのプロジェクトステータスが実際に変わった場合にのみ更新
- プロジェクト状態が変わるたびに`.codex/project-memory/<project_id>.md`を更新
...
| プロジェクトが再インポートされ続ける | `.codex/project-memory/registry.yaml`が存在し、正しいリポジトリルートを指しているか確認 |
