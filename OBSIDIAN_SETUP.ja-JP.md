...
Claude Scholarが`.opencode/project-memory/registry.yaml`を含むリポジトリ内で実行されている場合、そのリポジトリをObsidianプロジェクトナレッジベースにバインド済みとして扱い、デフォルトで更新を行います。
...
1. `.opencode/project-memory/registry.yaml`のプロジェクト設定
...
リポジトリが`.opencode/project-memory/registry.yaml`を通じて既にバインドされている場合、Claude Scholarは自動メンテナンスを控えめに行います:

- ターンが研究状態を変更した場合、常に`Daily/YYYY-MM-DD.md`を確認
- `00-Hub.md`はトップレベルのプロジェクトステータスが実際に変わった場合にのみ更新
- プロジェクト状態が変わるたびに`.opencode/project-memory/<project_id>.md`を更新
...
| プロジェクトが再インポートされ続ける | `.opencode/project-memory/registry.yaml`が存在し、正しいリポジトリルートを指しているか確認 |
