...
When Claude Scholar is running inside a repository that contains `.opencode/project-memory/registry.yaml`, it should treat the repository as bound to an Obsidian project knowledge base and update it by default.
...
When a repo is already bound through `.opencode/project-memory/registry.yaml`, Claude Scholar should keep automatic maintenance conservative:
...
- update `.opencode/project-memory/<project_id>.md` whenever project state changes,
...
| Project keeps re-importing | Check `.opencode/project-memory/registry.yaml` exists and points to the correct repo root |
