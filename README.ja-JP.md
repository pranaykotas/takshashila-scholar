...
- **2026-03-18**: **実験結果レポート、ライティングメモリ、README の整列** — `results-analysis` / `results-report` の二層構造を維持し、前者は厳密統計と実際の科研図、後者は意思決定志向の実験後レポートを担当する形を明確化しました。Obsidian への書き戻しは維持し、旧 `data-analyst` 入口は製品ストーリーから削除しました。さらに `paper-miner` の出力を共有ライティングメモリに統合し、`ml-paper-writing` と `review-response` が共通参照するように整理しました。同時に Codex 分岐 README の構造を主線に寄せつつ、Codex 固有の使い方は保持しています。
- **2026-03-17**: **Obsidian プロジェクト知識ベース** — ファイルシステム中心の Obsidian ワークフローを Codex 版へ移植し、プロジェクト導入、repo バインド後の自動同期、安定知識の `Papers / Knowledge / Experiments / Results / Writing` への振り分け、ラウンド単位の実験レポートの `Results/Reports/` 保存をサポートしました。Obsidian 側で MCP は不要です。
- **2026-02-26**: **Zotero MCP Web API モード** — リモート Zotero アクセス、DOI/arXiv/URL 取込、コレクション管理、アイテム更新に対応し、Codex `config.toml` 向けの設定説明も追加しました。
...
- **2026-02-11**: 大型アップデート — 研究 skills、agents、学術ワークフローのカバー範囲を拡張
