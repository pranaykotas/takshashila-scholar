...
- **2026-03-18**: **実験結果レポート、ライティングメモリ、ワークフロー整理** — 実験後の処理を二層に明確化しました。`results-analysis` は厳密統計、実際の科研図、`analysis-report` / `stats-appendix` / `figure-catalog` を担当し、`results-report` は意思決定志向の実験まとめレポートと Obsidian への書き戻しを担当します。冗長な `data-analyst` 入口を削除し、`/analyze-results` をデフォルトの一発分析 + 成稿コマンドへ調整しました。あわせて `paper-miner` 向けのグローバル writing memory と新しい `/mine-writing-patterns` を導入し、`ml-paper-writing` と `review-response` が共通の記憶を読むようにしました。README の位置づけも、人間の意思決定を中心に据えた半自動研究アシスタントであることがより明確になるよう更新し、ロゴも刷新しました。
- **2026-03-17**: **Obsidian プロジェクト知識ベース** — ファイルシステム中心のプロジェクト知識ベースワークフローに基づき、プロジェクト導入、バインド済み repo の自動同期、安定知識の `Papers / Knowledge / Experiments / Results / Writing` へのルーティング、ラウンド単位の実験レポートの `Results/Reports/` 保存をサポートします。MCP 依存はありません。
- **2026-02-26**: **Zotero MCP Web API モード** — リモート Zotero アクセス、DOI/arXiv/URL 取込、コレクション管理、アイテム更新に対応し、Claude Code、Codex CLI、OpenCode それぞれの設定説明も補足しました。
